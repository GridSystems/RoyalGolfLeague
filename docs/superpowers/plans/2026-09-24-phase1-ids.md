...
1. **A phone with the old app still open after go-live** — its inserts are refused (loud), but its *updates* use old IDs and match 0 rows (silent no-op). Expected: no one loses a score. Mitigation owned by Task 3: the guard refuses to run while any round dated today (Copenhagen) is incomplete and was created in the last 6 hours; tested in Task 3. Go-live checklist (Task 12) asks the user to confirm nobody is scoring.
